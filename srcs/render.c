		...
		SDL_Surface *texture)
{
	...
	{
		pixel_dist = render->win_h / fabs(render->jump_vision - renderer_y);
		...
		const t_u_range range)
{
	...
	while (i < range.end)
	{
        y = fabs(((i - (render->jump_vision) + render->wall_height / 2)
...
static t_render	fill_render_struct(t_env *e, Uint32 renderer_x)
{
	...
	render.x = renderer_x;
	render.jump_vision = e->p.vision_height + e->p.jump.height;
...
void		draw(t_env *e, t_collisions *node, Uint32 renderer_x)
{
	...
	while (node)
	{
		...
		range = wall_range(r.wall_height, r.jump_vision, r.win_h);
