...
ifeq ($(shell uname), Darwin)
	...
	CONFIGURE_SDL2_MIXER = cd ../$(SDL2_MIXER) && ./configure \
		--prefix="/Users/$(USER)/$(SDL2_MIXER)" && $(MAKE) -j && $(MAKE) install
	CONFIGURE_SDL2_TTF = cd ../$(SDL2_MIXER) && ./configure \
	...
	CONFIGURE_SDL2_MIXER = cd ../$(SDL2_MIXER) && ./configure \
	    && $(MAKE) -j && sudo $(MAKE) install
	CONFIGURE_SDL2_TTF = cd ../$(SDL2_TTF) && ./configure \
	    && $(MAKE) -j && sudo $(MAKE) install
...
$(NAME): $(OBJS)
	if [ ! -d $(SDL2) ] || [ ! -d $(SDL2_MIXER) ] || [ ! -d $(SDL2_TTF) ]; \
	    then $(EXTRACT); fi
	if [ ! -d $(SDL2)/build ] || [ ! -d $(SDL2_MIXER)/build ] \
	    || [ ! -d $(SDL2_TTF)/build ]; then $(CONFIGURE_SDL2) && \
	        $(CONFIGURE_SDL2_MIXER) && $(CONFIGURE_SDL2_TTF); fi
	$(MAKE) -j -C libft
...
$(NAME_EDITOR): $(OBJS_EDITOR)
	if [ ! -d $(SDL_PATH)/$(SDL2) ] || [ ! -d $(SDL_PATH)/$(SDL2_MIXER) ] \
	    || [ ! -d $(SDL_PATH)/$(SDL2_TTF) ]; \
	    then $(EXTRACT); fi
	if [ ! -d $(SDL_PATH)/$(SDL2)/build ] \
	    || [ ! -d $(SDL_PATH)/$(SDL2_MIXER)/build ] \
	        || [ ! -d $(SDL_PATH)/$(SDL2_TTF)/build ]; then $(CONFIGURE_SDL2) \
	        && $(CONFIGURE_SDL2_MIXER) && $(CONFIGURE_SDL2_TTF); fi
	$(MAKE) -j -C libft
